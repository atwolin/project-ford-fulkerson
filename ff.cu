...
__global__ void dev_is_frontier_empty_or_sink_found(bool* frontier, int N, int sink_pos, bool* found){
    int id = threadIdx.x;
    if (id < N) {
        if (frontier[id]) {
            atomicExch(*found, (id == sink_pos);
        }
    }
}
...
int main(int argc, char** argv){
    ...
    bool* d_is_empty_or_found;
    cudaMalloc(&d_is_empty_or_found, sizeof(bool));
    cudaMemcpy(d_is_empty_or_found, &found_augmenting_path, sizeof(bool), cudaMemcpyHostToDevice);
    ...
    do {
        ...
        while(!found_augmenting_path) {
                ...
                dev_is_frontier_empty_or_sink_found<<<1, 1>>>(d_frontier, N, sink, d_is_empty_or_found);
                cudaMemcpy(&found_augmenting_path, d_is_empty_or_found, sizeof(bool), cudaMemcpyDeviceToHost);
