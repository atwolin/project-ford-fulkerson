...
__global__ void find_augmenting_path(u_short* residual_capacity, Node_info* node_info, bool* frontier, bool* visited,
	...
	// shared memory
	extern __shared__ u_char shared_mem[];
	u_short *shared_residual_capacity = (u_short *)shared_mem;
	Node_info* shared_node_info = (Node_info*)(shared_residual_capacity + blockDim.x * total_nodes);
    bool* shared_frontier = (bool*)(shared_node_info + blockDim.x);
    bool* shared_visited = (bool*)(shared_frontier + blockDim.x);
    u_int* shared_locks = (u_int*)(shared_visited + blockDim.x);

    if (node_id < total_nodes) {
        shared_frontier[threadIdx.x] = frontier[node_id];
        shared_visited[threadIdx.x] = visited[node_id];
        shared_node_info[threadIdx.x] = node_info[node_id];

        for (int i = 0; i < total_nodes; ++i) {
            shared_residual_capacity[threadIdx.x * total_nodes + i] =
                residual_capacity[node_id * total_nodes + i];
        }

        shared_locks[threadIdx.x] = locks[node_id];
    }
    __syncthreads();
	...
	*/
	if (!shared_frontier[sink] && node_id < total_nodes && shared_frontier[threadIdx.x]) {
        shared_frontier[threadIdx.x] = false;
        shared_visited[threadIdx.x] = true;

        Node_info* neighbour;
        Node_info current_node_info = shared_node_info[threadIdx.x];
        u_int capacity;

		for (u_int i = node_id; i < total_nodes; ++i) {
            if (shared_frontier[i] || shared_visited[i] ||
                ((capacity = shared_residual_capacity[threadIdx.x * total_nodes + i]) <= 0)) {
                continue;
            }

            if (atomicCAS(&shared_locks[i], 0, 1) == 1 || shared_frontier[i]) {
                continue;
            }

            shared_frontier[i] = true;
            shared_locks[i] = 0;

            neighbour = &shared_node_info[i];
            neighbour->parent_index = node_id;
            neighbour->potential_flow = min(current_node_info.potential_flow, capacity);
        }

        for (u_int i = 0; i < total_nodes; ++i) {
            if (shared_frontier[i] || shared_visited[i] ||
                ((capacity = shared_residual_capacity[threadIdx.x * total_nodes + i]) <= 0)) {
                continue;
            }
    ...
    if (node_id < total_nodes) {
        frontier[node_id] = shared_frontier[threadIdx.x];
        visited[node_id] = shared_visited[threadIdx.x];
        node_info[node_id] = shared_node_info[threadIdx.x];

        for (int i = 0; i < total_nodes; ++i) {
            residual_capacity[node_id * total_nodes + i] =
                shared_residual_capacity[threadIdx.x * total_nodes + i];
        }
        locks[node_id] = shared_locks[threadIdx.x];
    }
}
